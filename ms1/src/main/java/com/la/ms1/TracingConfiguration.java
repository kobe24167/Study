...
public class TracingConfiguration extends WebMvcConfigurerAdapter {
	...
	@Bean
	Sender sender() {
		return OkHttpSender.create("http://116.196.97.97:9411/api/v2/spans");
	}
	...
	Tracing tracing(@Value("${spring.application.name}") String serviceName) {
		return Tracing.newBuilder().localServiceName(serviceName)
				...
				.currentTraceContext(
						ThreadLocalCurrentTraceContext.newBuilder().addScopeDecorator(MDCScopeDecorator.create()) // puts
																													// trace
																													// IDs
																													// into
																													// logs
